...
save_path = f"{main_dir}/PART2/pymodeller_pdb/"
...
with open(mut_path, "r") as file:
    ...
    for line in file:
        ...
        to_append = [pdb, residue, aa, chained]
        modeller_format.append(to_append)

for t in modeller_format:
    #first argument
    pdb, respos, restyp, chain, = t
    modelname = f"{pdbexp_path}{pdb}.pdb"
    if not os.path.exists(f"{save_path}{pdb}_{respos}{seq1(restyp)}.pdb"):
        ...
        #give a proper name
        mdl1.write(file=f"{save_path}{pdb}_{chain}_{respos}{seq1(restyp)}.pdb")
