...
pdbexp_path = f"{main_dir}/PART6/pdb_noh/"
multi_path = f"{main_dir}/PART1/DA_th1.0_s669_0.mut"
save_path = f"{main_dir}/PART6/pdb_modeller_0/"
...
def modeller_multi_mut(save_path, pdbexp_path, modeller_form):
    ...
    for t in modeller_form:
        ...
        full_mut = ""
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", mutations)
        for k,l in mutations:
            full_mut += f"_{k}{seq1(l)}"
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", full_mut)
        saving = f"{save_path}{pdb}_{chain}{full_mut}.pdb"
        ...
        for respos, restyp in mutations:
            if not os.path.exists(saving):
                modelname = f"{pdbexp_path}{pdb}.pdb"
