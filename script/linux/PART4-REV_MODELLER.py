...
pdbexp_path = f"{main_dir}/PART3/pymodeller_pdb/"
multi_path = f"{main_dir}/PART1/Protherm_multiple_pdbnum.mut"
save_path = f"{main_dir}/PART4/pymodeller_reverse/"

#for s2648 reverse mutations:
#pdbexp_path = f"{main_dir}/PART2/pymodeller_pdb/"
#multi_path = f"{main_dir}/PART1/S2648_renum_pdbnum.mut"
#save_path = f"{main_dir}/PART4/pymodeller_reverse_s2648/"
...
def modeller_multi_mut(save_path, pdb_path, modeller_form):
    '''
    apply multiple mutation from a pdb file

    save_path = the directory absolute path where you want to save the resulting file
    pdbexp_path = the directory absolute path where the original pdb file you want to use
    modeller_form = file and mutation data retrieved with the mut_data() function
    '''
    for t in modeller_form:
        #first argument
        pdb, mutations, chain, = t
        full_mut = ""
        orig_name = ""
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", mutations)
        for u,k,l in mutations:
            full_mut += f"_{k}{seq1(l)}"
            orig_name += f"_{k}{seq1(u)}"
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", full_mut)
        #the saving file has the same name of original file to simplify confrontation in later steps. I know it can be a little confusing but the mutation is applied correctly
        saving = f"{save_path}{pdb}_{chain}{orig_name}"
        start_file = f"{pdb_path}{pdb}_{chain}{orig_name}.pdb"
        ...
        # by using the same name of the original file to save the new one, the following part is useless
        #count = 0
        #if os.path.exists(f"{saving}.pdb"):
            #saving = f"{saving}_X{count}"
            #count += 1

        for e, respos, restyp in mutations:
            if not os.path.exists(f"{saving}.pdb"):
                modelname = start_file
            elif os.path.exists(f"{saving}.pdb"):
                modelname = f"{saving}.pdb"
            ...
            #give a proper name
            mdl1.write(file=f"{saving}.pdb")

            #delete the temporary file
            os.remove(modelname+restyp+respos+'.tmp')


modeller_form = rev_mut_data(multi_path)
modeller_multi_mut(save_path, pdbexp_path, modeller_form)