...
pdbexp_path = f"{main_dir}\\PART6\\pdb_modeller_0\\"
multi_path = f"{main_dir}\\PART1\\DA_th1.0_s669_1.mut"
save_path = f"{main_dir}\\PART6\\pdb_modeller_1\\"
...
def modeller_multi_mut(save_path, pdbexp_path, modeller_form):
    '''
    ...
    for t in modeller_form:
        ...
        full_mut = ""
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", mutations)
        for h,k,l in mutations:
            full_mut += f"_{k}{seq1(l)}"
            prev_mut += f"_{k}{seq1(h)}"
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", full_mut)
        saving = f"{save_path}{pdb}_{chain}{prev_mut}{full_mut}.pdb"
        true_path = f"{pdbexp_path}{pdb}_{chain}{prev_mut}.pdb"
        #print(">>>>>>>>>>>>>>>>>>>>>>>>", true_path)
        ...
        for ori_aa, respos, restyp in mutations:
            if not os.path.exists(saving):
                modelname = f"{pdbexp_path}{pdb}_{chain}{full_mut}.pdb"
            elif os.path.exists(saving):
                modelname = saving
            ...
            #give a proper name
            mdl1.write(file=saving)

            #delete the temporary file
            os.remove(modelname+restyp+respos+'.tmp')


modeller_form = mut_data(multi_path)
